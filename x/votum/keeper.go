...
// NewKeeper returns a votum keeper. It handles:
// - submitting votum proposals
...
func (keeper Keeper) IterateActiveProposalsQueue(ctx sdk.Context, endTime time.Time, cb func(proposal Proposal) (stop bool)) {
	...
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		proposalID, _ := SplitActiveProposalQueueKey(iterator.Key())
...
func (keeper Keeper) IterateInactiveProposalsQueue(ctx sdk.Context, endTime time.Time, cb func(proposal Proposal) (stop bool)) {
	...
	defer iterator.Close()
	for ; iterator.Valid(); iterator.Next() {
		proposalID, _ := SplitInactiveProposalQueueKey(iterator.Key())
		proposal, found := keeper.GetProposal(ctx, proposalID)
		if !found {
			panic(fmt.Sprintf("proposal %d does not exist", proposalID))
		}
...
func (keeper Keeper) IterateAllDeposits(ctx sdk.Context, cb func(deposit Deposit) (stop bool)) {
	store := ctx.KVStore(keeper.storeKey)
	iterator := sdk.KVStorePrefixIterator(store, DepositsKeyPrefix)
...
func (keeper Keeper) ActiveProposalQueueIterator(ctx sdk.Context, endTime time.Time) sdk.Iterator {
	store := ctx.KVStore(keeper.storeKey)
	return store.Iterator(ActiveProposalQueuePrefix, sdk.PrefixEndBytes(ActiveProposalByTimeKey(endTime)))
}
...
func (keeper Keeper) InactiveProposalQueueIterator(ctx sdk.Context, endTime time.Time) sdk.Iterator {
	store := ctx.KVStore(keeper.storeKey)
	return store.Iterator(InactiveProposalQueuePrefix, sdk.PrefixEndBytes(InactiveProposalByTimeKey(endTime)))
}
