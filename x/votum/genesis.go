...
func DefaultGenesisState() GenesisState {
	minDepositTokens := sdk.TokensFromConsensusPower(1)
	return GenesisState{
...
func ValidateGenesis(data GenesisState) error {
	...
	if threshold.IsNegative() || threshold.GT(sdk.OneDec()) {
		return fmt.Errorf("votum vote threshold should be positive and less or equal to one, is %s",
			threshold.String())
	}

	veto := data.TallyParams.Veto
	if veto.IsNegative() || veto.GT(sdk.OneDec()) {
		return fmt.Errorf("votum vote veto threshold should be positive and less or equal to one, is %s",
			veto.String())
	}

	if !data.DepositParams.MinDeposit.IsValid() {
		return fmt.Errorf("votum deposit amount must be a valid sdk.Coins amount, is %s",
